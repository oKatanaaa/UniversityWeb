...
public class LibraryDB {
    ...
    // Tables creation
    private static final String CREATE_READER_TABLE = "CREATE TABLE READER" +
            "(" +
    ...
    private static final String CREATE_SHELF_TABLE = "CREATE TABLE SHELF" +
            "(" +
            "AUTHOR VARCHAR(20) PRIMARY KEY" +
            ");";

    private static final String CREATE_READERSHELF_TABLE = "CREATE TABLE READERSHELF" +
            "(" +
    ...
    // INSERT transactions
    private static final String INSERT_READER = "INSERT INTO READER VALUES(?, ?, ?, ?)";      // (int, str, str, str)
    private static final String INSERT_SHELF = "INSERT INTO SHELF VALUES(?)";                 // (str)
    private static final String INSERT_READERSHELF = "INSERT INTO READERSHELF VALUES(?, ?)";  // (int, str)
    private static final String INSERT_BOOK = "INSERT INTO BOOK VALUES(?, ?, ?, ?, ?)";       // (str, int, int, int, str)
    ...
    private static final String SELECT_BOOK_TITLE = "SELECT * FROM BOOK WHERE TITLE=?";
    private static final String SELECT_BOOK_AUTHOR = "SELECT * FROM BOOK WHERE AUTHOR=?";
    private static final String SELECT_READERS_SHELVES = "SELECT S.AUTHOR FROM " +
            "SHELF S, READERSHELF RS, READER R " +
            "WHERE R.LOGIN=? AND R.PASSWORD=? AND R.READER_ID=RS.READER_ID";
    private static final String SELECT_READER_ID_NAME = "SELECT READER_ID, NAME FROM READER WHERE " +
            "LOGIN=? AND PASSWORD=?";


    // For checking whether an object is present in the database
    // ADDITIONAL TRANSACTION-2 ACCORDING TO THE TASK
    private static final String CHECK_BOOK = "SELECT COUNT(*) FROM BOOK WHERE TITLE=?";
    private static final String CHECK_READER = "SELECT COUNT(*) FROM READER WHERE " +
            "LOGIN=? AND PASSWORD=?";
    ...
    public void addShelf(ShelfImpl shelf, int reader_id) throws SQLException {
    ...
    public void addUser(Reader reader) throws SQLException {
        ...
        for(int i = 0; i < n_shelves; i++) {
            ShelfImpl shelf = reader.getShelf(i);
    ...
    public ShelfImpl getShelf(String author) throws SQLException {
        ShelfImpl shelf = new ShelfImpl(author);
    ...
    public ArrayList<ShelfImpl> getShelves(String login, String password) throws SQLException {
        if (!this.checkReader(login, password)) {
            throw new RuntimeException("User was not found.");
        }

        ArrayList<ShelfImpl> shelves = new ArrayList<>();
        ...
        ResultSet set = statement.getResultSet();
        while(set.next()) {
            ShelfImpl shelf = this.getShelf(set.getString("AUTHOR"));
            shelves.add(shelf);
        }
        return shelves;
    }

    public Reader getReader(String login, String password) throws SQLException {
        ArrayList<ShelfImpl> shelves = this.getShelves(login, password);
        PreparedStatement statement = connection.prepareStatement(LibraryDB.SELECT_READER_ID_NAME);
        statement.setString(1, login);
        statement.setString(2, password);
        statement.execute();
        ResultSet set = statement.getResultSet();
        ...
        Reader reader = new Reader(id, name, login, password);
        reader.setShelves(shelves);
        return reader;
    }
}
